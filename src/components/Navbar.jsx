...
function Navbar() {
  const [toggleMenu, setTogglMenu] = React.useState(false);
  return (
    <nav className="flex items-center justify-between w-full p-4 md:justify-center">
      ...
      <ul className="flex-row items-center justify-between flex-initial hidden text-white list-none md:flex">
        ...
        <li className="bg-[#2952e3] py-2 px-7 mx-4 rounded-full cursor-pointer hovor:bg-[#2546bd]">
          Login
        </li>
      </ul>
      <div className="relative flex">
        {toggleMenu ? (
          <AiOutlineClose
            ...
            onClick={() => setTogglMenu(false)}
          />
        ) : (
          <HiMenuAlt4
            fontSize={28}
            className="text-white cursor-pointer md:hidden"
            onClick={() => setTogglMenu(true)}
          />
        )}
        {toggleMenu && (
          <ul
          ...
          >
            <li className="w-full my-2 text-xl">
              <AiOutlineClose onClick={() => setTogglMenu(false)} />
            </li>
